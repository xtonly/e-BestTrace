...
# 融合 eTraffic 1.0 UI 布局，新增 ICMP/TCP 协议切换
...
# ================== 全局状态变量 ==================
TRACE_PROTO="ICMP"
# 修复核心：NextTrace 默认即为 ICMP，不需要也不存在 -I 参数
PROTO_FLAG=""
...
run_tests() {
    ...
    for ((i=0; i<len; i++)); do
        ...
        if $should_run; then
            ...
            # 规范命令结构：将 target_ip 放在参数最后，并调用 -d ipinfo 防护限制
            nexttrace $PROTO_FLAG -q 1 -M -d ipinfo "$target_ip" | tee /tmp/nt_temp.log
...
run_custom_test() {
    ...
    nexttrace $PROTO_FLAG -q 1 -M -d ipinfo "$custom_ip" | tee /tmp/nt_temp.log
